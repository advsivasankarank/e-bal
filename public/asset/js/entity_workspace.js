 ...
 * Handles tabs, auditor/director search, entity category selection,
 * CIN rules, MCA lookup, and form interactions.
 */
(function() {
    'use strict';
    ...
    window.selectCategory = function(value) {
        ...
        for (var i = 0; i < chips.length; i++) {
            chips[i].classList.toggle('active', chips[i].getAttribute('data-value') === value);
        }
        ...
        fetch(ebalBaseUrl + 'company_dashboard/mca_lookup.php?type=' + encodeURIComponent(type) + '&category=' + encodeURIComponent(cat) + '&identifier=' + encodeURIComponent(identifier))
            ...
            .then(function(result) {
                ...
                Object.keys(fields).forEach(function(id) {
                    ...
                    if (el && !el.value.trim()) el.value = fields[id];
                });
                applyCinRules();
                statusEl.style.background = '#d1fae5'; statusEl.textContent = 'Master data fetched. Review before saving.';
            })
    ...
    if (auditorSearch) {
        auditorSearch.addEventListener('input', function() {
            ...
            auditorTimeout = setTimeout(function() {
                fetch(ebalBaseUrl + 'company_dashboard/ajax_auditor_search.php?q=' + encodeURIComponent(q))
                    .then(function(r) { return r.json(); })
                    .then(function(data) {
                        if (!data.ok || !data.auditors.length) { auditorResults.innerHTML = '<div class="emw-search-item"><span class="emw-search-item-name">No auditors found</span></div>'; auditorResults.classList.add('open'); return; }
                        var html = '';
                        data.auditors.forEach(function(a) {
                            html += '<div class="emw-search-item" onclick="selectAuditor(' + a.auditor_id + ', \'' + esc(a.firm_name) + '\', \'' + esc(a.partner_name) + '\', \'' + esc(a.frn) + '\', \'' + esc(a.membership_no) + '\')">';
        ...
        auditorSearch.addEventListener('blur', function() {
            setTimeout(function() { auditorResults.classList.remove('open'); }, 200);
        });
    }

    window.selectAuditor = function(id, firm, partner, frn, membership) {
        document.getElementById('governance_auditor_id').value = id;
        document.getElementById('auditor-display-name').textContent = firm;
        document.getElementById('auditor-display-meta').textContent = partner + ' · FRN: ' + frn + ' · M.No: ' + membership;
    ...
    window.clearAuditor = function() {
        document.getElementById('governance_auditor_id').value = '';
    ...
    if (directorSearch) {
        directorSearch.addEventListener('input', function() {
            ...
            directorTimeout = setTimeout(function() {
                fetch(ebalBaseUrl + 'company_dashboard/ajax_director_search.php?q=' + encodeURIComponent(q))
                    .then(function(r) { return r.json(); })
                    .then(function(data) {
                        if (!data.ok || !data.directors.length) { directorResults.innerHTML = '<div class="emw-search-item"><span class="emw-search-item-name">No directors found</span></div>'; directorResults.classList.add('open'); return; }
                        var html = '';
                        data.directors.forEach(function(d) {
                            var alreadyAdded = addedDirectors.indexOf(d.director_id) !== -1;
                            html += '<div class="emw-search-item" onclick="' + (alreadyAdded ? '' : 'addDirector(' + d.director_id + ', \'' + esc(d.director_name) + '\', \'' + esc(d.din) + '\')') + '" style="' + (alreadyAdded ? 'opacity:0.5;cursor:default;' : '') + '">';
                            html += '<div class="emw-search-item-name">' + esc(d.director_name) + (alreadyAdded ? ' (added)' : '') + '</div>';
                            html += '<div class="emw-search-item-meta">DIN: ' + esc(d.din) + '</div>';
                            html += '</div>';
                        });
        ...
        directorSearch.addEventListener('blur', function() {
            setTimeout(function() { directorResults.classList.remove('open'); }, 200);
        });
    }
    ...
    window.addDirector = function(id, name, din) {
        ...
        var noDir = document.getElementById('no-directors');
        if (noDir) noDir.style.display = 'none';

        var row = document.createElement('div');
        row.className = 'emw-director-row';
        row.setAttribute('data-director-id', id);
        row.innerHTML =
            '<div><strong>' + esc(name) + '</strong><br><span style="font-size:0.72rem;color:#64748b;">DIN: ' + esc(din) + '</span></div>' +
            '<div><input type="text" name="director_designations[]" placeholder="Designation" style="width:100%;padding:6px 8px;border:1px solid #d1d5db;border-radius:4px;font-size:0.82rem;"></div>' +
            '<div><input type="text" name="director_signing_orders[]" placeholder="Order" style="width:100%;padding:6px 8px;border:1px solid #d1d5db;border-radius:4px;font-size:0.82rem;"></div>' +
            '<div><input type="text" name="director_signing_auths[]" placeholder="Signing Authority" style="width:100%;padding:6px 8px;border:1px solid #d1d5db;border-radius:4px;font-size:0.82rem;"></div>' +
        ...
        directorResults.classList.remove('open');
    };

    window.removeDirector = function(btn, id) {
        var row = btn.closest('.emw-director-row');
        if (row) row.remove();
        var idx = addedDirectors.indexOf(id);
        if (idx !== -1) addedDirectors.splice(idx, 1);
        if (addedDirectors.length === 0) {
            var noDir = document.getElementById('no-directors');
            if (noDir) noDir.style.display = '';
        }
    };

    /* ---- Create New Director ---- */
    window.showNewDirectorForm = function() {
        document.getElementById('new-director-form').style.display = '';
    };

    window.hideNewDirectorForm = function() {
        document.getElementById('new-director-form').style.display = 'none';
    };
    ...
    window.createAndAddDirector = function() {
        ...
        if (!name) { alert('Director name is required.'); return; }
        ...
        .then(function(data) {
            if (data.ok) {
                addDirector(data.director_id, name, din);
                hideNewDirectorForm();
                document.getElementById('new_dir_name').value = '';
                document.getElementById('new_dir_din').value = '';
                document.getElementById('new_dir_pan').value = '';
            } else {
                alert(data.message || 'Failed to create director.');
            }
        });
    };
    ...
    window.fetchFromTallyBridge = function() {
        fetch('http://127.0.0.1:9123/company', {
            method: 'POST',
        ...
        .then(function(data) {
            ...
            if (nameField && c.name) nameField.value = c.name || c.mailing_name || '';
        })
        .catch(function() {});
    };
